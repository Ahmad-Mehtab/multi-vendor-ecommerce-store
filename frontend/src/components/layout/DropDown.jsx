...
const handleSubmit = (cate) => {
  ...
  return (
    <div className='pb-4 w-[270px] bg-[#fff] absolute z-30 rounded-md mt-[2px] border border-gray-600 shadow-sm'>
        {categoriesData && categoriesData.map((cate,index) => (
            <div key={index} onClick={()=> handleSubmit(cate)} className={`${styles.noramlFlex} gap-5 p-1 hover:bg-slate-300 select-none`}>
